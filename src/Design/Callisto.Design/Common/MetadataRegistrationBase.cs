...
namespace Callisto.Design.Common
{
	public class MetadataRegistrationBase
	{
				...
				try
				{
						...
						name.StartsWith("P:", StringComparison.OrdinalIgnoreCase))
					{
						...
						if (t != null && t.IsPublic && t.IsClass &&
                            t.IsSubclassOf(Callisto.Design.Types.PlatformTypes.DependencyObjectType))
						{
		...
		{
			var attrs = t.GetCustomAttributes(Callisto.Design.Types.PlatformTypes.EditorBrowsableAttributeType, false);
			foreach (var attr in attrs)
			{
                return Callisto.Design.Types.PlatformTypes.IsBrowsable(attr);
			}
			return true;
		}
		...
		{
            var attrs = pi.GetCustomAttributes(Callisto.Design.Types.PlatformTypes.EditorBrowsableAttributeType, false);
			foreach (var attr in attrs)
			{
                return Callisto.Design.Types.PlatformTypes.IsBrowsable(attr);
			}
